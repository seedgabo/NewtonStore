...
export class HomePage {
  ...
  getProgramaciones() {
    this.api
      .get(
        `programacion-pedidos?whereDate[fecha]=${"tomorrow"}&where[cliente_id]=${
          this.api.user.cliente_id
        }&afterEach[setProductos]=&afterEach[setCategorias]=`
      )
      .then((data: any) => {
        console.log(data);
        data.forEach(element => {
          if (element.tipo == "almuerzo") this.progamacion.almuerzo = element;
          if (element.tipo == "comida") this.progamacion.comida = element;
          if (element.tipo == "cena") this.progamacion.cena = element;
        });
      })
      .catch(err => {
        console.error(err);
  ...
  ordenar(tipo) {
    ...
    if (!this.entidad_id) {
      this.toast
        .create({
          duration: 3000,
          message: "Elija un lugar de entrega primero",
          position: "top"
        })
        .present();
      return;
    ...
    if (
      this.filterEntidad(this.entidad_id).length > 0 &&
      this.sub_entidad_id == null
    ) {
      this.toast
        .create({
          duration: 3000,
          message: "Elija un ubicacion de entrega primero",
          position: "top"
        })
        .present();
      return;
    ...
    if (this.entidad_id == 18 && tipo != "cena") {
      this.toast
        .create({
          duration: 3000,
          message: "Esta planta solo puede pedir cena",
          position: "top"
        })
        .present();
      return;
  ...
  getPedidos() {
    ...
    this.api
      .get(
        "pedidos?with[]=items&&whereDateBetween[created_at]=today,tomorrow&where[user_id]=" +
          user.id
      )
      .then((data: Array<any>) => {
  ...
  getPedidosAyer() {
    ...
    this.api
      .get(
        "pedidos?with[]=items&&whereDateBetween[created_at]=yesterday,today&where[user_id]=" +
          user.id
      )
      .then((data: Array<any>) => {
        ...
        data.forEach(pedido => {
          ...
          if (pedido.tipo == "comida") this.api.pedidos_ayer.comida = pedido;
          if (pedido.tipo == "cena") this.api.pedidos_ayer.cena = pedido;
        });
      })
      .catch(err => {
        console.warn(err);
    ...
    this.api
      ...
      .then((data: Array<any>) => {
        ...
        this.entidades.forEach(ent => {
          if (ent.codigo && ent.codigo.indexOf("*") > -1) array.push(ent.id);
        });
  ...
  canOrder(tipo = null) {
    if (
      (this.api.cupon && this.status[tipo] == false) ||
      (this.status.comida === false &&
        this.status.cena === false &&
        this.status.almuerzo === false)
    )
      return true;
    return false;
  }
  ...
  selectClientes() {
    var modal = this.modal.create(Selector, {
      uri: "users",
      append: "limit=50&where[cliente_id]=" + this.api.user.cliente_id,
  ...
  verCupon(numero) {
    this.api
      .get("cupones?where[code]=" + numero + "&scope[valido]")
      .then((data: Array<any>) => {
        console.log(data);
        if (data.length != 0) {
          ...
          this.alert
            .create({ buttons: ["Ok"], message: "cupon invalido" })
            .present();
        }
      })
      .catch(err => {
        this.alert
          .create({ buttons: ["Ok"], message: "Error al aplicar el cupon" })
          .present();
      });
  }
  ...
  generateCupon() {
    this.api
      ...
      .then((cupon: any) => {
        ...
        this.alert
          .create({
            message: "Codigo Cupon",
            title: cupon.code,
            subTitle: "Copiado en el  portapapeles",
            buttons: ["OK"]
          })
          .present();
      })
      .catch(err => {
        this.alert
          .create({ title: "No se pudo generar el cupon", buttons: ["OK"] })
          .present();
      });
  }

  goTocomedor() {
    this.navCtrl.push("PickComedorPage");
  }
}
