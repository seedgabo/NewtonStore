...
export class PedidoPage {
  ...
  processCarrito() {
    ...
    data.fecha_envio = moment.utc().add(1, 'days').toDate().toISOString().substring(0, 10);
    data.fecha_entrega = moment.utc().add(1, 'days').toDate().toISOString().substring(0, 10);
    data.fecha_pedido = moment.utc().add(1, 'days').toDate().toISOString().substring(0, 10);
    if (this.api.cupon && this.delivery == 'today') {
      data.fecha_envio = moment.utc().toDate().toISOString().substring(0, 10);
      data.fecha_entrega = moment.utc().toDate().toISOString().substring(0, 10);
      data.fecha_pedido = moment.utc().toDate().toISOString().substring(0, 10);
    }
    ...
    this.api.post("pedidos", data)
      .then((data) => {
        ...
        this.api.storage.set('last_pedido', moment.utc().format('Y-M-D'));
        console.log(data);
      })
      .catch((err) => {
        loading.dismiss().then(() => {
